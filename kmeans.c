...
void free_malloc2(struct vector *vec)
{
    /*runs through each cord in each vector and frees the allocated memory*/
    struct vector *temp_vec;
    struct cord *temp_cord;
...
struct cord* initialize_cord(int d)
{
...
struct vector* initialize_centroids(int k, int d) {
    ...
    for (i = 1; i <= k; i++) {
        ...
        curr_vec->cords = initialize_cord(d);
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i<=iter; i++)
    {
        ...
        {
            avg_cord = initialize_cord(d);
            if (avg_cord == NULL)
            {
                free_malloc(head_vec); /*frees memory before code termination*/
